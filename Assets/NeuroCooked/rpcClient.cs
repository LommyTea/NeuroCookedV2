...
public class rpcClient : MonoBehaviour
{
    ...
    private CVEPdummyScript.CVEPdummyScriptClient client;
    ...
    {
        var request = new addSeqDataRequest() { SequenceNum = sequenceNumber, Duration = duration };
        var call = client.addSeqDataAsync(request);
    }
    ...
    private IEnumerator training()
    {
        ...
        var call = client.trainingModelAsync(request);
    ...
    private IEnumerator decode()
    {
        ...
        var request = new Empty();
        var call = client.decodeChoiceAsync(request);
        yield return new WaitUntil(() => call.ResponseAsync.IsCompleted);
        if (call.ResponseAsync.IsCompletedSuccessfully)
        {
            var response = call.ResponseAsync.Result;
